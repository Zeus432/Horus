...
class Horus(commands.Bot):
    def __init__(self, CONFIG: dict, *args, **kwargs) -> None:
        ...
        self.owner_ids = frozenset(
            CONFIG["Owners"]
        )  # I like freezing my bot owner ids you can remove this frozenset if you want to
            ...
            if pre := await self.redis.hget("prefix", message.guild.id):
                ...
                if pre := await self.db.fetchval(
                    ...
                    prefix = await self.db.fetchval(
                        f"INSERT INTO guilddata(guildid, prefix) VALUES($1, $2) ON CONFLICT (guildid) DO UPDATE SET prefix = $2 RETURNING prefix",
                        message.guild.id,
                        prefix,
                    )

                await self.redis.hset(
                    "prefix", message.guild.id, prefix[0]
    ...
    async def on_ready(self) -> None:
        ...
        await self.redis.hmset(
            "prefix", {"default": self._config.get("prefix")}
        )  # make redis prefix cache

        await self.redis.delete("blacklist")  # clear redis blacklist cache

        blguilds = [
            guildid
            for guildid in await self.db.fetchval(
                "SELECT guildid FROM guilddata WHERE blacklists->'blacklisted' @> 'true'"
            )
        ]  # get blacklisted guilds
        blusers = [
            userid
            for userid in await self.db.fetchval(
                "SELECT userid FROM userdata WHERE blacklists->'blacklisted' @> 'true'"
            )
        ...
        await self.change_presence(
            ...
            activity=discord.Activity(
                type=discord.ActivityType.watching, name=f"for @{self.user.name} help"
            ),
        )
    ...
    async def setup_hook(self) -> None:
        try:  # Try Connecting to Postgres
            ...
            self.db = await asyncpg.create_pool(
                self._config.get("postgresuri"), init=init_connection
            )
        ...
        try:  # Try connecting to Redis
            self.redis = await redis.from_url(
                self._config.get("redisuri"), decode_responses=True
            )
    ...
    ) -> None:  # Process commands when owner edits a message
        if not (
            after.channel.permissions_for(after.guild.me).send_messages
            and after.channel.permissions_for(after.guild.me).embed_links
        ):
            return
    ...
    async def restartchk(
        self, message: List[int], invoke: List[int], start: int
    ) -> None:
        ...
        try:
            ...
            await msg.edit(
                content=f"Restarted **{self.user.name}** in `{round(end - start, 2)}s`"
            )
        except:
            pass
    ...
    async def on_message(self, message: discord.Message):
        ...
        if (
            await self.redis.lpos("blacklist", message.author.id) is not None
